...
三周目当前状态：开启中(部分)
- [进入](mqqapi://aio/inlinecmd?command=${encodeURIComponent(
          `/lapnext`
          ...
          keyboard: {
            content: {
              rows: [
                {
                  buttons: [
                    button(0, "✔️Yes", `Y`, userId, "1"),
                    button(2, "❌No", "N", userId, "2"),
                  ],
                },
              ],
            },
  ...
  ctx
    ...
    .action(async ({ session }) => {
      ...
      if (user.level < 80 || user.ultramonster.length < 5)
        return `条件不满足，请升级至80级，并且拥有5只传说中的宝可梦`;
      const playerList: PokemonList = await getList(
        session.userId,
        ctx,
        user.monster_1
            ...
            markdown: {
              ...
              params: [
                ...
                },
                {
                  key: config.key6,
                  values: [
                    "但是你的宝可梦将会保留\r并且开启传说中的宝可梦收集",
                  ],
                },
                {
                  key: config.key7,
                  values: [
                    "当某个传说中的宝可梦收集至100%后，将可以捕捉并杂交",
                  ],
                },
              ],
            },
