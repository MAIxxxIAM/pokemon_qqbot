...
export async function apply(ctx: Context) {
  ...
  const idiomsFilePath = path.join(
    dirname,
    "./pokedle/src/assets",
    "宝可兜",
    "idioms.json"
  );
  const pinyinFilePath = path.join(
    dirname,
    "./pokedle/src/assets",
    "宝可兜",
    "pinyin.json"
...
![img#20px #20px](${await toUrl(
      ctx,
      session,
      `${config.图片源}/sr/346.png`
    )}) : ${player.ultra["346.346"] * 10}% ${
      "🟩".repeat(Math.floor(player.ultra["346.346"] / 2)) +
...
输出：`,
  ...
  function isLengthCharacterIdiom(
    targetIdiom: string,
    length: number
  ): boolean {
