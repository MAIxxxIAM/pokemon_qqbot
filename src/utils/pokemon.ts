...
const pokemonCal = {
  power(a: string[], b: number, c1: PokemonList, id: string) {
    ...
    const nature =
      index == -1
        ? { effect: "无", up: 0, down: 0 }
        : c1.pokemon[index].natures;
    const powerNatrue =
      index == -1
        ? {
            id: "临时",
    ...
    try {
      ...
      for (let i = 0; i < a.length; i++) {
        c[i] = String(
          Math.round(((Number(a[i]) + 30 + powerUp[i] / 8) * b) / 50 + 5)
        );
      }
      c[nature.up] = String(
        Math.ceil(Number(c[nature.up]) * (1 + powerNatrue.natureLevel * 0.01))
      );
      c[nature.down] = String(
        Math.floor(
          Number(c[nature.down]) * (1 - powerNatrue.natureLevel * 0.01)
        )
      );
      return c;
    } catch (e) {
  ...
  exp_bar(a: number, b: number) {
    ...
    const expBar =
      "🟩".repeat(Math.floor(nowExp / 10)) +
      "🟨".repeat(nowExp % 10 > 4 ? 1 : 0) +
      "⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜".substring(Math.round(nowExp / 10));
    return expBar;
  },
  ...
  pokemomPic(a: string, b: boolean) {
    try {
      ...
      if (b) {
        return h.image(
          `${config.图片源}/fusion/${pokemon_a}/${pokemon_a}.${pokemon_b}.png`
        );
      } else {
  ...
  skillinfo(a: string[], c: string, b: boolean = false) {
    ...
    skill = skill.sort((a, b) => {
      return skillMachine.skill[a].Dam - skillMachine.skill[b].Dam;
    });
    let skillInfo = [];
    for (let j = 0; j < skill.length; j++) {
      skillInfo.push(
        `${skillMachine.skill[skill[j]].skill}:${
          skillMachine.skill[skill[j]].Dam
        } 类型:${
          skillMachine.skill[skill[j]].category === 1 ? "物理" : "特殊"
        } 属性:${skillMachine.skill[skill[j]].type}`
      );
    }
