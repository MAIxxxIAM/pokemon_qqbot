...
export async function apply(ctx: Context) {
  ctx
    ...
    .action(async ({ session }) => {
      ...
      do {
        randomTrainer = {
          ...trainer_list[Math.floor(Math.random() * trainer_list.length)],
        };
      } while (
        player.trainer_list.some((trainer) => trainer.tid === randomTrainer.tid)
      );
      const md = `恭喜<@${session.userId}>获得了训练家${randomTrainer.name}！
![img#80px #80px](${await toUrl(
        ctx,
        session,
        `${config.图片源}/trainers/${randomTrainer.source_name}.png`
      )})
---
是否改名？`;
      const kb = {
...
- [点击更换](mqqapi://aio/inlinecmd?command=${encodeURIComponent(
        `/更换训练师 ${randomTrainer.name}`
      )}&reply=false&enter=true)
- [继续开启盲盒](mqqapi://aio/inlinecmd?command=${encodeURIComponent(
        `/盲盒`
      )}&reply=false&enter=true)`;
      try {
        await sendMarkdown(ctx, md2, session);
      } catch {
        const md3 = `<@${session.userId}>成功收集训练家${
          randomTrainer.source_name
        }！
...
- [继续开启盲盒](mqqapi://aio/inlinecmd?command=${encodeURIComponent(
  ...
  ctx
    .command("宝可梦")
    .subcommand("训练师改名 <tid:number> [newName:text]", "给你的训练师改名")
    .action(async ({ session }, tid: number, newName: string) => {
      const [player]: Pokebattle[] = await ctx.database.get("pokebattle", {
        id: session.userId,
      });
      if (!player) {
      ...
      const isTrainer = player.trainer_list.find(
        (trainers) => trainers.tid == tid
      );
      if (!isTrainer) {
...
> 当前名字中含有违禁词`;
      try {
        await sendMarkdown(ctx, md, session);
        await ctx.database.set("pokebattle", { id: session.userId }, (row) => ({
          trainer_list: player.trainer_list.map((trainers) =>
            trainers.tid == tid ? { ...trainers, name: newName } : trainers
          ),
        }));
      } catch {
  ...
  ctx
    .command("宝可梦")
    .subcommand("更换训练师 [trainer:text]", "更换你的训练师形象")
    ...
    .action(async ({ session, options }, trainer: string) => {
      ...
      if (!trainer && !options.tid) {
        ...
        let listContent = `训练师列表 (${options.page}/${Math.ceil(
          trainerList.length / 10
        )}) ：
---
`;
        // for (let index = startIndex; index < endIndex; index++) {
        //   const element = trainerList[index];
        //   listContent += `\n> ${element.tid} ![img#50px #50px](${await toUrl(
        //     ctx,
        //     session,
        //     `${config.图片源}/trainers/${element.source_name}.png`
        //   )}) ${element.name}`;
        // }
        const trainerImage = await ctx.canvas.render(600, 1200, async (c) => {
          ...
          c.fillStyle = "#FFFFFF";
          c.fillText("训练师列表", 300, 50);
          c.font = "bold 30px Arial";
          c.fillText("编号", 50, 100);
          c.fillText("训练师", 150, 100);
          c.fillText("名称", 400, 100);
          for (let index = startIndex; index < endIndex; index++) {
            const element = trainerList[index];
            c.fillText(
              element.tid.toString(),
              50,
              170 + 100 * (index - page * 10)
            );
            c.drawImage(
              ...
              110,
              120 + 100 * (index - page * 10),
              100,
              100
            );
            c.fillText(element.name, 400, 170 + 100 * (index - page * 10));
          }
        });
        listContent += `![img#300px #600px](${await toUrl(
          ctx,
          session,
          trainerImage.attrs.src
        )})`;
        const md =
          listContent +
          `
...
点击按钮后输入名称更换训练师`;
        ...
        try {
          ...
          listContent = `训练师列表 (${options.page}/${Math.ceil(
            trainerList.length / 10
          )}) ：
---`;
          for (let index = startIndex; index < endIndex; index++) {
...
点击按钮后输入名称更换训练师`;
      ...
      const isTrainer = options.tid
        ? trainerList.find((trainers) => trainers.tid == Number(options.tid))
        : trainerList.find((trainers) => trainers.name == trainer);
      if (!isTrainer) {
...
![img#500px #500px](${await toUrl(
        ctx,
        session,
        `${config.图片源}/trainers/${isTrainer.source_name}.png`
      )})`;
      const kb = {
        keyboard: {
          content: {
            rows: [
              {
                buttons: [
                  button(0, "盲盒", "/盲盒", session.userId, "6"),
                  button(0, "更换训练师", "/更换训练师", session.userId, "2"),
                ],
              },
              {
                buttons: [
