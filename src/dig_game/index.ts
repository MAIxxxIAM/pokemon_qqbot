...
export async function apply(ctx: any) {
  ctx
    .command("宝可梦")
    .subcommand("化石挖掘", "挖掘化石相关指令")
    .subcommand("dig [position:text]")
    .alias("挖掘")
    .action(async ({ session }, position: string) => {
      ...
      berryBag.fertilizes += berryBag.fertilizes > 100 ? 0 : 2;
      berryBag.fertilizes =
        berryBag.fertilizes > 100 ? 100 : berryBag.fertilizes;
      const [digGames]: DigChannel[] = await ctx.database.get(
        "pokemon.digChannel",
        session.channelId
      );
      ...
      if (!position || !digGames) {
        ...
        try {
          stats = digGame.dig(P[0], P[1], player.tool);
        } catch (e) {
          return "挖掘失败";
        }
        const dest = calculateDistance(
          digGame.item.x,
          digGame.item.y,
          P[0],
          P[1]
      ...
      stats?.isFindItem
        ? await ctx.database.remove("pokemon.digChannel", [session.channelId])
      ...
      if (stats?.isFindItem) {
        ...
        if (!isFind) {
          player.fossil_bag.push({
            id: digGame.item.id,
            name: digGame.item.name,
            score: digGame.item.score,
            number: 1,
          });
        }
...
> 镐子可以探测化石位置，绿色为4格内，黄色为5到7格，红色为8格以上
...
[${
        player.tool === 1 ? "🔨" : "⛏"
      } 切换工具](mqqapi://aio/inlinecmd?command=${encodeURIComponent(
        `/切换工具`
...
当前肥料:${berryBag.fertilizes}`;
            ...
            rows: [
              {
                buttons: [
                  button(
                    2,
                    `${stats?.isFindItem ? "继续游戏" : "挖掘"}`,
                    `/挖掘 `,
                    session.userId,
                    "6",
                    stats?.isFindItem ? true : false
                  ),
                  button(2, `售出`, `/售出 `, session.userId, "p"),
                ],
              },
              {
                buttons: [
                  button(2, `融合化石`, `/融合化石 `, session.userId, "r"),
                ],
              },
            ].concat(
              stats?.isFindItem
      ...
      try {
        digGames?.msg_id
          ? await session.bot.deleteMessage(session.channelId, digGames.msg_id)
          : null;
      } catch (e) {}
      await ctx.database.set(
        "pokemon.digChannel",
  ...
  ctx
    ...
    .subcommand("化石挖掘", "挖掘化石相关指令")
    .subcommand("dig")
    .subcommand("售出 <id:text>", { minInterval: 2000 })
    .action(async ({ session }, id: string) => {
      ...
      if (!id) {
        ...
        player.fossil_bag.forEach((f) => {
          ...
          if (!fossilBtn?.buttons) {
            fossilBtn.buttons = [];
          }
          if (fossilBtn?.buttons.length >= 2) {
            i++;
          }
          fossilBtn.buttons = fossilBtn?.buttons.concat(
            button(
              0,
              `${f.name}x${f.number}`,
              `/售出 ${f.id}`,
      ...
      return `你售出了${fossil.name}，获得了${
        fossil.score * (player.lap >= 3 ? 1 : 10)
      }${player.lap >= 3 ? "积分" : "金币"}`;
    });
...
![img#20px #20px](${await toUrl(
        ctx,
        session,
        `${config.图片源}/sr/341.png`
      )}) : ${player.ultra["341.341"] * 10}% ${
        "🟩".repeat(Math.floor(player.ultra["341.341"] / 2)) +
        "🟨".repeat(player.ultra["341.341"] % 2) +
        "⬜⬜⬜⬜⬜".substring(Math.round(player.ultra["341.341"] / 2))
      }
